...
@Component({
  selector: 'app-whiteboard4',
  templateUrl: './whiteboard4.component.html',
  styleUrl: './whiteboard4.component.scss',
...
export class Whiteboard4Component {
  ...
  color = input<string>('#0c62f0');
  width = input<number>(6);
  mode = input<DrawMode>('draw');

  // Internal state synced from inputs
  private currentColor = signal('#0c62f0');
  private strokeWidth = signal(6);
  private currentMode = signal<DrawMode>('draw');

  // Track paths for save functionality
  paths = signal<Path[]>([]);

  constructor() {
    // Sync inputs to internal signals
    effect(() => this.currentColor.set(this.color()));
    effect(() => this.strokeWidth.set(this.width()));
    effect(() => this.currentMode.set(this.mode()));
  }
  ...
  // File operations - public method to load SVG content
  loadSvgContent(svgText: string): void {
    try {
